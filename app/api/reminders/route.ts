...
export async function GET() {
  ...
  const { data, error } = await supabase.from("reminders").select("id, task_id, scheduled_at, active, created_at, updated_at").eq("owner_id", user.id).order("scheduled_at", { ascending: true });
...
export async function POST(request: Request) {
  ...
  const { data, error } = await supabase.from("reminders").insert({ owner_id: user.id, task_id: task.id, scheduled_at: parsed.data.scheduledAt, active: parsed.data.active }).select("id, task_id, scheduled_at, active, created_at, updated_at").single();
