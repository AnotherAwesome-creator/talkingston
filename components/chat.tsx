...
export function ChatExperience() {
  ...
  const [searchResults, setSearchResults] = useState<Array<{ id: string; title: string; snippets: string[] }>>([]);
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      void fetch(`/api/conversations/search?q=${encodeURIComponent(search.trim())}`)
        .then((response) => response.ok ? response.json() as Promise<{ results: typeof searchResults }> : Promise.reject(new Error("Search failed.")))
        .then((data) => setSearchResults(data.results))
        .catch(() => setError("Unable to search conversations."));
    }, 300);
    return () => window.clearTimeout(timer);
  }, [search]);
